...
public class TicketLineInfo implements SerializableWrite, SerializableRead, Serializable {
    ...
    private boolean toKitchenPrinter;
    ...
    public TicketLineInfo(String productid, double dMultiply, double dPrice, TaxInfo tax, Properties props) {
        init(productid, null, dMultiply, dPrice, tax, props, false);
    }

    public TicketLineInfo(String productid, double dMultiply, double dPrice, TaxInfo tax) {
        init(productid, null, dMultiply, dPrice, tax, new Properties(), false);
    }
    ...
    public TicketLineInfo(String productname, String producttaxcategory, double dMultiply, double dPrice, TaxInfo tax) {
        ...
        props.setProperty("product.name", productname);
        props.setProperty("product.taxcategoryid", producttaxcategory);
        init(null, null, dMultiply, dPrice, tax, props, false);
    }

    public TicketLineInfo() {
        init(null, null, 0.0, 0.0, null, new Properties(), false);
    }

    public TicketLineInfo(ProductInfoExt product, double dMultiply, double dPrice, TaxInfo tax, Properties attributes) {

        String pid;
        boolean toKichtenPrinter = false;
        
        if (product == null) {
            pid = null;
        } else {
            pid = product.getID();
            toKichtenPrinter = product.isKitchenPrint();
        ...
        init(pid, null, dMultiply, dPrice, tax, attributes, toKichtenPrinter);
    }
    ...
    public TicketLineInfo(TicketLineInfo line) {
        init(line.productid, line.attsetinstid, line.multiply, line.price, line.tax, (Properties) line.attributes.clone(), line.isKitchenPrint());
    }

    private void init(String productid, String attsetinstid, double dMultiply, double dPrice, TaxInfo tax, Properties attributes, boolean toKitchenPrinter) {
        ...
        this.toKitchenPrinter = toKitchenPrinter;
